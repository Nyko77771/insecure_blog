...
class Blog():
    ...
    def get_all_user_blogs(username):
        db = DatabaseConnection()
        query = "SELECT b.*, u.username FROM blogs b JOIN users u ON b.user_id = u.id WHERE u.username = %s ORDER BY b.created_at DESC"

        try:
            result = db.execute_select_query(query, (username,))
            # VULNERABILITY: Too much information
            if result:
                print(f"All user blogs retrieved. Results: {result}")
                return result
            else:
    ...
    def get_blog_by_id(search_id):
        db = DatabaseConnection()
        query = "SELECT * FROM blogs b WHERE b.id = %s"
        try:
            result = db.execute_select_query(query, (search_id,))
            if result:
                print(f"Found blog by id.")
                return result
            else:
    ...
    def search(search_word):
        ...
        try:
            result = db.execute_select_query(query, (search_word,))
            if result:
